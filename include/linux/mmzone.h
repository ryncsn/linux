 ...
 * through file descriptors will be categoried as higher tier as shown below:
 *
 ...
 *    2      0     | - Reclaimable workingset, still on tier 0 to avoid over-protect. [4]
 ...
 * --RPOMOTION--->-/
 * 
 * 1. The tier is order_base_2(N-1) for N > 1, 0 otherwise. Ideally each tier
 ...
 *
 * 2. Folios access for at lease once are still on tier 0:
 * (folio_lru_refs(folio) == LRU_REFS_REFERENCED == 1).
 * One time usage doesn't make the folio qualified to be protected in anyway.
 *
 * 3. Folios there are accessed more than once are considered workingset:
 ...
 * LRU lock so it's cheap. It won't be promoted furture if it's not on
 * oldest gen to avoid over-promoting of caches.
 *
 * 4. A folio got qualified as the workingset is still on the tire 0 unless it
 * is access again. That makes the workingset still easily reclaimable, since
 * the initial move in (2) and the promotion in (6) already protests recent
 ...
 * tier will be sacrifieced to protect higher tier when the higher tier have a
 ...
 * but we will promote file caches more aggresively, and maybe unexpectedly.
 *
 ...
 *
 * A folios referenced count never go backwards except a gen increasement
 * in (7) or a promotion / protect will reset a folios with higher referenced
