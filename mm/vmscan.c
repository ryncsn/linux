...
static bool folio_promote_lru_refs(struct folio *folio)
{
	...
	return refs > LRU_REFS_REFERENCED;
}
#else
static bool folio_promote_lru_refs(struct folio *folio)
{
						  ...
						  struct scan_control *sc)
{
	...
	referenced_folio = folio_test_clear_referenced_bit(folio);
...
static int folio_inc_gen(struct lruvec *lruvec, struct folio *folio, bool reclaiming)
{
	...
	do {
		...
		if (old_gen != lru_gen_from_seq(lrugen->min_seq[type]))
			return old_gen;
		...
		if (memchr_inv(lruvec->lrugen.nr_pages, 0,
			...
			for (int gen = 0; gen < MAX_NR_GENS; gen++) {
				...
				for (int zone = 0; zone < MAX_NR_ZONES; zone++)
					pr_err("%10ld ", atomic_long_read(&lruvec->lrugen.nr_pages[gen][0][zone]));
				pr_err("file: ");
				for (int zone = 0; zone < MAX_NR_ZONES; zone++)
					pr_err("%10ld ", atomic_long_read(&lruvec->lrugen.nr_pages[gen][1][zone]));
				pr_err("\n");
			}
			WARN_ON(1);
		}
