...
struct swap_cluster_info {
	spinlock_t lock; /* Protects all fields below other than list */
	u16 count;
	u8 flags;
	u8 order;
	swp_te_t *table;
 ...
 * cluster. swap_lock_folio_cluster is preferred when possible
 */
static __always_inline struct swap_cluster_info *__swap_lock_cluster(
...
#define swap_lock_cluster(si, off) __swap_lock_cluster(si, off, false)
