...
#define SWAP_COUNT_SHIFT	SWP_TE_COUNT_BITS
...
static inline swp_te_t folio_swp_te(struct folio *folio)
{
	BUILD_BUG_ON(SWAP_CACHE_PFN_BITS >
		     (BITS_PER_LONG - SWP_TE_PFN_SHIFT - SWP_TE_COUNT_BITS));
	swp_te_t swp_te = {
		.counter = (folio_pfn(folio) << SWP_TE_PFN_SHIFT) | SWP_TE_PFN_MARK
	};
	return swp_te;
}
...
{
	swp_te_t swp_te = { .counter = SWP_TE_BAD };
	return swp_te;
}
...
{
	return ((swp_te.counter & SWP_TE_PFN_LOW_MASK) == SWP_TE_PFN_MARK);
}
...
{
	return swp_te.counter == SWP_TE_BAD;
}

static inline bool swp_te_is_countable(swp_te_t ent)
{
...
static inline struct folio *swp_te_folio(swp_te_t swp_te)
{
	...
	return pfn_folio((swp_te.counter & SWP_TE_PFN_MASK) >> SWP_TE_PFN_SHIFT);
}
...
static inline swp_te_t swp_te_set_count(swp_te_t swp_te,
					unsigned char count)
{
	VM_BUG_ON(!swp_te_is_countable(swp_te));
	VM_BUG_ON(count > SWP_TE_COUNT_MAX);

	swp_te.counter &= ~SWP_TE_COUNT_MASK;
