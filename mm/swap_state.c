				 ...
				 struct folio *old, struct folio *new)
{
	...
	do {
		VM_BUG_ON(swp_te_folio(__swap_table_get(ci, offset)) != old);
		__swap_table_set(ci, offset, folio_swp_te(new));
	} while (++offset < end);
}
...
{
	pgoff_t offset = swp_offset(entry);
	return swp_te_shadow(__swap_table_get(swp_cluster(entry), offset));
}
...
{
	struct folio *folio;
	folio = swp_te_folio(__swap_table_get(swp_cluster(entry),
					      swp_offset(entry)));
		...
		bool skip_if_exists)
{
	...
	/*
	 * First check the swap cache, caller will check if the
	 * folio is still valid.
	 */
