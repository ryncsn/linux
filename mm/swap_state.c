				 ...
				 struct folio *old, struct folio *new)
{
	...
	VM_BUG_ON(offset == end);
	do {
		VM_BUG_ON(entry_to_folio(__swap_table_get(ci, offset)) != old);
		__swap_table_set(ci, offset, folio_to_entry(new));
	} while (++offset < end);
}
			       ...
			       struct folio *old, struct folio *new)
{
	...
	VM_BUG_ON(offset == end);
	do {
		if (entry_to_folio(__swap_table_get(ci, offset)) != old)
			return -ENOENT;
		__swap_table_set(ci, offset, folio_to_entry(new));
	} while (++offset < end);
			 ...
			 void **shadow)
{
	...
	do {
		if (!__swap_count(swp_info(entry), offset)) {
	...
	VM_BUG_ON(offset == end);
	do {
		__swap_table_set(ci, offset, folio_to_entry(folio));
	} while (++offset < end);
	swap_unlock_cluster(ci);
			    ...
			    struct folio *folio, void *shadow)
{
	...
	do {
		...
		__swap_table_set(ci, offset, shadow_to_entry(shadow));
...
{
	pgoff_t offset = swp_offset(entry);
	return entry_to_shadow(__swap_table_get(swp_cluster(entry), offset));
}
...
void __swap_cache_clear_shadow(swp_entry_t entry, int nr_ents)
{
	...
	VM_BUG_ON(offset == end);
	do {
		if (entry_to_folio(__swap_table_get(ci, offset)))
			continue;
		__swap_table_set(ci, offset, null_to_entry());
	} while (++offset < end);
}
...
{
	return !!entry_to_folio(__swap_table_get(ci, offset));
}
...
{
	struct folio *folio;
	folio = entry_to_folio(__swap_table_get(swp_cluster(entry),
					      swp_offset(entry)));
 ...
 * this raced with another users, only one user will successfully add it's
 * folio into swap cache, and it will be returned for all readers.
 *
 * If folio is a large folio, continues swap entries will be read.
 */
