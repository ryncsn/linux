				...
				struct folio *folio, unsigned int start, unsigned int order)
{
	...
	if (folio) {
		/* from folio_alloc_swap, add to swap cache directly */
		for (offset = start; offset < end; offset++) {
				   ...
				   struct swap_cluster_info *ci, gfp_t gfp)
{
	...
	pr_emerg("DEBUG: Allocating\n");
	table = kzalloc(sizeof(unsigned long) * SWAPFILE_CLUSTER, gfp);
	if (!table) {
		pr_emerg("DEBUG: Allocating failed!!!\n");
		return -ENOMEM;
	}
					   ...
					   unsigned long offset)
{
	...
	VM_WARN_ON(!count);
	VM_WARN_ON(count > ENTRY_COUNT_MAX);
...
next_cluster:
	...
	do {
		...
		if (WARN_ON_ONCE(!count)) {
			...
			/* XXX: Maybe just clean the shadow here? */
...
int folio_alloc_swap(struct folio *folio, gfp_t gfp)
{
	...
	if (mem_cgroup_try_charge_swap(folio, folio->swap)) {
		folio->swap.val = SWAP_ENTRY_INVALID;
		delete_from_swap_cache(folio);
	}
		       ...
		       unsigned long start, unsigned int nr_pages)
{
	...
	ci->count -= nr_pages;
	do {
		VM_WARN_ON_ONCE(swp_te_get_count(__swap_table_get(ci, offset)));
 ...
 * in, caller must ensure the entries won't get complete released during this
 ...
 *
 * Context: The caller must ensure the entry won't be completely release during
