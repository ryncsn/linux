	...
	*has_cache = entry_is_folio(entry);
	do {
		...
		if (*has_cache != entry_is_folio(entry))
			return false;
		if (count != entry_get_count(entry))
			return false;
	} while (++offset < end);
				  ...
				  unsigned long start, unsigned long end)
{
	...
	spin_unlock(&ci->lock);
	do {
		entry = __swap_map_get(ci, offset);
		VM_BUG_ON(entry_is_shadow(entry));
		if (entry_is_folio(entry) && !entry_get_count(entry)) {
...
static bool cluster_alloc_range(struct swap_info_struct *si, struct swap_cluster_info *ci,
				unsigned int offset, unsigned char usage,
				unsigned int order)
{
	unsigned int nr_pages = 1 << order;
	unsigned int end = offset + nr_pages;
	...
	do {
		VM_BUG_ON(usage > SWAP_CONT_MAX);
		__swap_map_set(ci, offset, entry_set_count(null_to_entry(), usage));
	} while (++offset < end);
...
static void swap_reclaim_full_clusters(struct swap_info_struct *si, bool force)
{
	...
	while ((ci = isolate_lock_cluster(si, &si->full_clusters))) {
		...
		while (offset < end) {
			entry = __swap_map_get(ci, offset);
			if (!entry_get_count(entry) && entry_is_folio(entry)) {
				spin_unlock(&ci->lock);
...
new_cluster:
	...
	if (order < SWAP_NR_PARTIAL_ORDERS) {
		 ...
		 * is not critical, while still keep the fragment cluster
		 * being rotated and scanned (for reclaiming HAS_CACHE).
		 */
			    ...
			    int order)
{
	...
	if (cluster_is_usable(ci, order)) {
		if (cluster_is_empty(ci))
			offset = cluster_offset(si, ci);
		found = alloc_swap_scan_cluster(si, ci, offset, order, 1);
		if (found)
...
start_over:
	plist_for_each_entry_safe(si, next, &swap_avail_heads[node], avail_lists[node]) {
		...
		if (get_swap_device_info(si)) {
			offset = cluster_alloc_swap_entry(si, order, 1);
			put_swap_device(si);
			if (offset) {
...
retry_cache:
	...
	VM_BUG_ON(!swap_entries_put_map(swp_info(entry), entry, size));

	ci = swap_lock_cluster(swp_info(entry), swp_offset(entry));
	// pr_err("Allocated %d@%lx, map %d, has_cache: %d\n", size,
	// 		swp_offset(entry),
	// 		swp_info(entry)->swap_map[swp_offset(entry)],
	// 		__swap_has_cache(ci, swp_offset(entry)));
	swap_unlock_cluster(ci);
	return 0;

out_free:
	swap_entries_put_map(swp_info(entry), entry, size);
	return -ENOMEM;
}
...
/*
 * Drop the last flag(1 or SWAP_MAP_SHMEM) of swap entries,
			      ...
			      unsigned long maxpages)
{
	...
	for (i = 0; i < nr_clusters; i++)
		swap_table_free(&cluster_info[i]);
	kvfree(cluster_info);
}
						...
						unsigned long maxpages)
{
	...
	for (i = 0; i < nr_clusters; i++) {
		...
		if (swap_table_alloc(&cluster_info[i]))
			goto err_free;
	}
