...
{
	WARN_ON(ci->table);
	ci->table = kzalloc(sizeof(swp_table_ent_t) * SWAPFILE_CLUSTER,
			    GFP_KERNEL);
	if (!ci->table)
		return -ENOMEM;
	return 0;
...
static void cluster_table_free(struct swap_cluster_info *ci)
{
	...
	for (offset = 0; offset <= SWAPFILE_CLUSTER; offset++)
		WARN_ON(!entry_is_null(__swap_table_get(ci, offset)));
...
int folio_alloc_swap(struct folio *folio, gfp_t gfp)
{
	 ...
	 */
retry_cache:
	err = swap_cache_add_folio(entry, folio, NULL);
	if (err) {
...
/*
 * Drop the last ref of swap entries,
