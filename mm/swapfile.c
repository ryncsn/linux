...
static int populate_cluster(struct swap_info_struct *si,
			    struct swap_cluster_info *ci,
			    int order)
{
	int ret;
				       ...
				       unsigned int offset)
{
	...
	if (likely(folio)) {
		...
		/* From get_swap_page_of_type: hibernation / others */
					      ...
					      struct folio *folio)
{
	...
	ci = isolate_lock_free_cluster(si, folio_order(folio));
	if (ci)
...
new_cluster:
	...
	if (si->flags & SWP_PAGE_DISCARD) {
		found = alloc_swap_scan_free_list(si, folio);
		if (found)
			goto done;
	}
...
/*
 * Put the refcount of an swap slot indicated by an swap entry.
 * This won't free the slot, caller would handle the freeing
 * for folio based batch freeing or range based batch freeing.
 */
					   ...
					   unsigned long offset)
{
	...
	VM_WARN_ON_ONCE(count > SWP_TE_COUNT_MAX);

	if (count == SWP_TE_COUNT_MAX) {
		count = ci->extend_table[offset % SWAPFILE_CLUSTER];
		VM_WARN_ON_ONCE(count < SWP_TE_COUNT_MAX);
		count--;
		if (count == (SWP_TE_COUNT_MAX - 1)) {
	...
	VM_WARN_ON(!count && !swp_te_is_folio(__swap_table_get(ci, offset)));
...
/*
 * Increase the refcount of an swap slot indicated by an swap entry.
 * Only slots assigned to a folio can be duplicated.
 */
				 ...
				 unsigned long offset)
{
	...
	if (WARN_ON_ONCE(count == SWP_TE_COUNT_BAD))
		return -ENOENT;
	...
	if (likely(count < (SWP_TE_COUNT_MAX - 1))) {
		__swap_table_set_count(ci, offset, count + 1);
		VM_WARN_ON_ONCE(ci->extend_table && ci->extend_table[offset % SWAPFILE_CLUSTER]);
	} else if (count == (SWP_TE_COUNT_MAX - 1)) {
		if (ci->extend_table) {
			VM_WARN_ON_ONCE(ci->extend_table[offset % SWAPFILE_CLUSTER]);
			ci->extend_table[offset % SWAPFILE_CLUSTER] = SWP_TE_COUNT_MAX;
			__swap_table_set_count(ci, offset, SWP_TE_COUNT_MAX);
		} else {
			return -ENOMEM;
		}
	} else if (count == SWP_TE_COUNT_MAX) {
...
bool swap_entry_swapped(struct swap_info_struct *si, swp_entry_t entry)
{
	...
	return swp_te_is_countable(swp_te) && swp_te_get_count(swp_te);
}
...
static void *swap_start(struct seq_file *swap, loff_t *pos)
{
	...
	for (type = 0; (si = swp_type_get_info(type)); type++) {
		if (!(si->flags & SWP_USED) || !si->cluster_info)
			continue;
		if (!--l)
			return si;
	}
